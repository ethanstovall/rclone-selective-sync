...
func (cs *ConfigManager) syncConfigToRemote() error {
	...
	dstFs := fmt.Sprintf("%s:%s", projectRemoteConfig.RemoteName, projectRemoteConfig.BucketName)

	if err := RcloneCopyFile(projectPath, "sync.json", dstFs, "sync.json"); err != nil {
