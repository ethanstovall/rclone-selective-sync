...
In [NewFolderDialog.tsx](../../frontend/src/components/SyncFolders/NewFolderDialog.tsx), users must manually type the folder path:
...
1. [ ] Add `OpenFolderPicker` method to FolderService (or create DialogService)
2. [ ] Update main.go to pass app reference to service
3. [ ] Regenerate TypeScript bindings
4. [ ] Update NewFolderDialog component with Browse button
5. [ ] Add loading state during folder selection
6. [ ] Add error handling for invalid selections
7. [ ] Auto-fill folder name from selected path
8. [ ] Test on all platforms (macOS, Windows, Linux)
